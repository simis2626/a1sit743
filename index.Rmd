...
sc<- specc(x=listSiteData, centers=4, iterations=600)
size(sc)
centers(sc)
withinss(sc)
siteData %>% ggplot(aes(x = V1, y=V2)) + geom_point(aes(color=cl$cluster)) + scale_color_gradientn(colours = rainbow(n = 4)) + labs(title="SiteData kmeans")
...
WTempData %>% ggplot(aes(x = water_temp)) + geom_histogram(bins = 50) + labs(title = "Water Temp Histogram, fitted model") #+  geom_smooth(method = "glm")
